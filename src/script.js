...
;(() => {
  ...
  class ButtonInteractions {
    ...
    setupActiveNavTracking(navMap) {
      ...
      const sectionTargets = entries
        .map(([label, marker]) => {
          ...
          return el
            ? { label, section: el.closest('section, div') || el }
            : null
        })
        .filter(Boolean)
    ...
    navigateTo(marker) {
      ...
      const targetElement = Array.from(document.querySelectorAll('.text')).find(
        (el) => el.textContent.includes(marker)
      )

      if (!targetElement) {
        notifications.show(
          'That section is coming soon. Thanks for checking!',
          'info'
        )
        return
      }
  ...
  function setupKeyboardNavigation(getCarousel) {
    document.addEventListener('keydown', (e) => {
      ...
      if (e.key === 'Escape') {
        document
          .querySelectorAll('.korre-notification')
          .forEach((n) => n.remove())
        return
      }
